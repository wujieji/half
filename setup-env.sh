...
install_python_tools() {
    if command -v python3 &> /dev/null; then
        ...
        # 安装pip包
        local pip_packages=(black flake8 pylint)
        for pkg in "${pip_packages[@]}"; do
            if ! python3 -m pip show "$pkg" &> /dev/null; then
                log_info "安装Python包: $pkg"
                python3 -m pip install --user "$pkg" || log_warn "$pkg安装失败"
            else
                log_info "Python包 $pkg 已安装"
            fi
        done
        return 0
    else
...
main() {
    ...
    run_install_step "Python工具" "install_python_tools"
